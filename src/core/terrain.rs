...
impl Terrain for DefaultTerrain {
    ...
    fn is_able_to_get_in (&self, db: &Database, entity: &dyn Entity, map: &Map,
                          current_pos: Position, next_pos: Position, from_dir: Direction)->bool {
        self.allow_from_dir().has_direction(from_dir)
    }
    fn is_able_to_get_out(&self, db: &Database, entity: &dyn Entity, map: &Map,
...
pub trait Terrain {
    ...
    /// -> bool: 侵出可能
    fn is_able_to_get_out(&self, db: &Database, entity: &dyn Entity, map: &Map, current_pos: Position, next_pos: Position, to_dir: Direction)->bool;
